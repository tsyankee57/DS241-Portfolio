...
df2017 = read_csv(here("data_raw/airline_data", "airline_data_2017.zip")) %>% clean_names()
df2018 = read_csv(here("data_raw/airline_data", "airline_data_2018.zip")) %>% clean_names()
df2019 = read_csv(here("data_raw/airline_data", "airline_data_2019.zip")) %>% clean_names()
df2020 = read_csv(here("data_raw/airline_data", "airline_data_2020.zip")) %>% clean_names()
df2021 = read_csv(here("data_raw/airline_data", "airline_data_2021.zip")) %>% clean_names()
df2022 = read_csv(here("data_raw/airline_data", "airline_data_2022.zip")) %>% clean_names()
...
df_complete = rbind(df2017,df2018,df2019,df2020,df2021,df2022) %>%
  filter(origin == "LGA" | dest == "LGA", carrier_name == "Southwest Airlines Co.", 
         passengers > 0) %>%
  select(departures_performed, passengers, carrier_name, origin, dest, year, month) %>%
  mutate(total_month = month + 12*(year - 2017))
df_complete %>% ggplot(aes(x = month)) + geom_bar(aes(weight = departures_performed)) +
  facet_wrap(~year) + labs(title = "Southwest Airlines Flights to/From La Guardia by Year",
                           x = "Month", y = "Departures Performed")
```
